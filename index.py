...
async def post_new_runs():
    channel = bot.get_channel(931584454156230687)
    ...
    if  new_call != newest_run:
        ...
        runner_is_in_server = discord.utils.get(channel.guild.members, name=newest_run["Runner"]) != None
        
        if runner_is_in_server:
            await channel.send(
                "**A new run has been verified !**\n"
                "GG {} for PB ! :partying_face:".format(discord.utils.get(channel.guild.members, name=newest_run["Runner"])), embed=embed_run
            )
        else:
            await channel.send(
                "**A new run has been verified !**\n"
        ...
        await channel.send(newest_run["Video Link"])
